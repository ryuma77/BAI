...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.bpj.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.bpjs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('jenis_program') ? 'is-invalid' : '' }}" name="jenis_program" id="jenis_program" required>
                    ...
                    @foreach(App\Models\Bpj::JENIS_PROGRAM_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('jenis_program', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('perusahaan') ? 'is-invalid' : '' }}" type="number" name="perusahaan" id="perusahaan" value="{{ old('perusahaan', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('karyawan') ? 'is-invalid' : '' }}" type="number" name="karyawan" id="karyawan" value="{{ old('karyawan', '') }}" step="0.01" required>

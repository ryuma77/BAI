    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.employees.update", [$employee->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tempat_lahir') ? 'is-invalid' : '' }}" type="text" name="tempat_lahir" id="tempat_lahir" value="{{ old('tempat_lahir', $employee->tempat_lahir) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tanggal_lahir') ? 'is-invalid' : '' }}" type="text" name="tanggal_lahir" id="tanggal_lahir" value="{{ old('tanggal_lahir', $employee->tanggal_lahir) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('jenis_kelamin') ? 'is-invalid' : '' }}" name="jenis_kelamin" id="jenis_kelamin" required>
                    ...
                    @foreach(App\Models\Employee::JENIS_KELAMIN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('jenis_kelamin', $employee->jenis_kelamin) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('division') ? 'is-invalid' : '' }}" name="division_id" id="division_id" required>
                    @foreach($divisions as $id => $division)
                        <option value="{{ $id }}" {{ (old('division_id') ? old('division_id') : $employee->division->id ?? '') == $id ? 'selected' : '' }}>{{ $division }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('position') ? 'is-invalid' : '' }}" name="position_id" id="position_id" required>
                    @foreach($positions as $id => $position)
                        <option value="{{ $id }}" {{ (old('position_id') ? old('position_id') : $employee->position->id ?? '') == $id ? 'selected' : '' }}>{{ $position }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('alamat') ? 'is-invalid' : '' }}" type="text" name="alamat" id="alamat" value="{{ old('alamat', $employee->alamat) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kota') ? 'is-invalid' : '' }}" type="text" name="kota" id="kota" value="{{ old('kota', $employee->kota) }}">
                @if($errors->has('kota'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kode_pos') ? 'is-invalid' : '' }}" type="text" name="kode_pos" id="kode_pos" value="{{ old('kode_pos', $employee->kode_pos) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('employee_status') ? 'is-invalid' : '' }}" name="employee_status" id="employee_status" required>
                    ...
                    @foreach(App\Models\Employee::EMPLOYEE_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('employee_status', $employee->employee_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>

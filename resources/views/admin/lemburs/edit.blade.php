    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.lemburs.update", [$lembur->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ip_address') ? 'is-invalid' : '' }}" type="text" name="ip_address" id="ip_address" value="{{ old('ip_address', $lembur->ip_address) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nik') ? 'is-invalid' : '' }}" name="nik_id" id="nik_id" required>
                    @foreach($niks as $id => $nik)
                        <option value="{{ $id }}" {{ (old('nik_id') ? old('nik_id') : $lembur->nik->id ?? '') == $id ? 'selected' : '' }}>{{ $nik }}</option>
                    @endforeach
                </select>
                @if($errors->has('nik'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tanggal') ? 'is-invalid' : '' }}" type="text" name="tanggal" id="tanggal" value="{{ old('tanggal', $lembur->tanggal) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jam_lembur') ? 'is-invalid' : '' }}" type="number" name="jam_lembur" id="jam_lembur" value="{{ old('jam_lembur', $lembur->jam_lembur) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('keterangan') ? 'is-invalid' : '' }}" type="text" name="keterangan" id="keterangan" value="{{ old('keterangan', $lembur->keterangan) }}">

    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sections.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('section_code') ? 'is-invalid' : '' }}" type="text" name="section_code" id="section_code" value="{{ old('section_code', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('department_name') ? 'is-invalid' : '' }}" name="department_name_id" id="department_name_id" required>
                    @foreach($department_names as $id => $department_name)
                        <option value="{{ $id }}" {{ old('department_name_id') == $id ? 'selected' : '' }}>{{ $department_name }}</option>
                    @endforeach
                </select>

    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.jadwals.update", [$jadwal->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tanggal_awal') ? 'is-invalid' : '' }}" type="text" name="tanggal_awal" id="tanggal_awal" value="{{ old('tanggal_awal', $jadwal->tanggal_awal) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tanggal_akhir') ? 'is-invalid' : '' }}" type="text" name="tanggal_akhir" id="tanggal_akhir" value="{{ old('tanggal_akhir', $jadwal->tanggal_akhir) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('departement') ? 'is-invalid' : '' }}" name="departement_id" id="departement_id" required>
                    @foreach($departements as $id => $departement)
                        <option value="{{ $id }}" {{ (old('departement_id') ? old('departement_id') : $jadwal->departement->id ?? '') == $id ? 'selected' : '' }}>{{ $departement }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bagian') ? 'is-invalid' : '' }}" name="bagian_id" id="bagian_id">
                    @foreach($bagians as $id => $bagian)
                        <option value="{{ $id }}" {{ (old('bagian_id') ? old('bagian_id') : $jadwal->bagian->id ?? '') == $id ? 'selected' : '' }}>{{ $bagian }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('jam_masuk') ? 'is-invalid' : '' }}" type="text" name="jam_masuk" id="jam_masuk" value="{{ old('jam_masuk', $jadwal->jam_masuk) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('jam_pulang') ? 'is-invalid' : '' }}" type="text" name="jam_pulang" id="jam_pulang" value="{{ old('jam_pulang', $jadwal->jam_pulang) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('kategori_jadwal') ? 'is-invalid' : '' }}" name="kategori_jadwal" id="kategori_jadwal">
                    ...
                    @foreach(App\Models\Jadwal::KATEGORI_JADWAL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('kategori_jadwal', $jadwal->kategori_jadwal) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>

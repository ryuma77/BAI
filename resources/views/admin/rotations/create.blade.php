...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.rotation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.rotations.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('department_to') ? 'is-invalid' : '' }}" name="department_to_id" id="department_to_id">
                    @foreach($department_tos as $id => $department_to)
                        <option value="{{ $id }}" {{ old('department_to_id') == $id ? 'selected' : '' }}>{{ $department_to }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('valid_date') ? 'is-invalid' : '' }}" type="text" name="valid_date" id="valid_date" value="{{ old('valid_date') }}">

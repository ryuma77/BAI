...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.tarikDataabsen.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.tarik-dataabsens.update", [$tarikDataabsen->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="ip_address">{{ trans('cruds.tarikDataabsen.fields.ip_address') }}</label>
                <input class="form-control {{ $errors->has('ip_address') ? 'is-invalid' : '' }}" type="text" name="ip_address" id="ip_address" value="{{ old('ip_address', $tarikDataabsen->ip_address) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nik') ? 'is-invalid' : '' }}" name="nik_id" id="nik_id" required>
                    @foreach($niks as $id => $nik)
                        <option value="{{ $id }}" {{ (old('nik_id') ? old('nik_id') : $tarikDataabsen->nik->id ?? '') == $id ? 'selected' : '' }}>{{ $nik }}</option>
                    @endforeach
                </select>
                @if($errors->has('nik'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tanggal') ? 'is-invalid' : '' }}" type="text" name="tanggal" id="tanggal" value="{{ old('tanggal', $tarikDataabsen->tanggal) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('jam_masuk') ? 'is-invalid' : '' }}" type="text" name="jam_masuk" id="jam_masuk" value="{{ old('jam_masuk', $tarikDataabsen->jam_masuk) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('jam_keluar') ? 'is-invalid' : '' }}" type="text" name="jam_keluar" id="jam_keluar" value="{{ old('jam_keluar', $tarikDataabsen->jam_keluar) }}" required>

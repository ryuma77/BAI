    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.divisions.update", [$division->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('division_code') ? 'is-invalid' : '' }}" type="text" name="division_code" id="division_code" value="{{ old('division_code', $division->division_code) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('division_name') ? 'is-invalid' : '' }}" type="text" name="division_name" id="division_name" value="{{ old('division_name', $division->division_name) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('business_unit') ? 'is-invalid' : '' }}" name="business_unit_id" id="business_unit_id" required>
                    @foreach($business_units as $id => $business_unit)
                        <option value="{{ $id }}" {{ (old('business_unit_id') ? old('business_unit_id') : $division->business_unit->id ?? '') == $id ? 'selected' : '' }}>{{ $business_unit }}</option>
                    @endforeach
                </select>

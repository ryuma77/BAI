    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.salaries.update", [$salary->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employee') ? 'is-invalid' : '' }}" name="employee_id" id="employee_id" required>
                    @foreach($employees as $id => $employee)
                        <option value="{{ $id }}" {{ (old('employee_id') ? old('employee_id') : $salary->employee->id ?? '') == $id ? 'selected' : '' }}>{{ $employee }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('basic_salary') ? 'is-invalid' : '' }}" type="number" name="basic_salary" id="basic_salary" value="{{ old('basic_salary', $salary->basic_salary) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bpjs') ? 'is-invalid' : '' }}" name="bpjs[]" id="bpjs" multiple>
                    @foreach($bpjs as $id => $bpjs)
                        <option value="{{ $id }}" {{ (in_array($id, old('bpjs', [])) || $salary->bpjs->contains($id)) ? 'selected' : '' }}>{{ $bpjs }}</option>
                    @endforeach
                </select>
                @if($errors->has('bpjs'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tunjangans') ? 'is-invalid' : '' }}" name="tunjangans[]" id="tunjangans" multiple>
                    @foreach($tunjangans as $id => $tunjangan)
                        <option value="{{ $id }}" {{ (in_array($id, old('tunjangans', [])) || $salary->tunjangans->contains($id)) ? 'selected' : '' }}>{{ $tunjangan }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('potongans') ? 'is-invalid' : '' }}" name="potongans[]" id="potongans" multiple>
                    @foreach($potongans as $id => $potongan)
                        <option value="{{ $id }}" {{ (in_array($id, old('potongans', [])) || $salary->potongans->contains($id)) ? 'selected' : '' }}>{{ $potongan }}</option>
                    @endforeach
                </select>
